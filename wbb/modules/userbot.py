...
@app2.on_message(
    filters.command("produk", prefixes=USERBOT_PREFIX)
    & ~filters.forwarded
    & ~filters.via_bot
    & SUDOERS
)
async def produkk(_, message: Message):
    # Create a transaction token
    transaction_token = snap.create_transaction_token(10000, "IDR")

    # Create a catalog product object
    product = snap.create_product(
        name="Test Product",
